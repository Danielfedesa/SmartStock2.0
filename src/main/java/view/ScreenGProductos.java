...
public class ScreenGProductos extends JFrame {
	...
	public ScreenGProductos(Producto producto) {
		...
		// Renderizador para la columna Stock que cambia de color si es menor que el
		// stock mínimo
		tablaProductos.getColumnModel().getColumn(4)
				.setCellRenderer((table, value, isSelected, hasFocus, row, column) -> {
					...
					// Si la celda está seleccionada, establecer el color de selección
					// predeterminado
					if (isSelected) {
		...
		tablaProductos.getColumnModel().getColumn(7).setCellEditor(new javax.swing.DefaultCellEditor(new JTextField()) {
			...
			public java.awt.Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected,
				...
				botonEditar.addActionListener(e -> {
					try {
						int idProducto = Integer.parseInt(table.getValueAt(row, 0).toString()); // Recuperar el ID del
																								// producto.
